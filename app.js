...
async function fetchGithubRepos() {
  ...
  if (!username) {
    githubStatus.textContent = "Enter a GitHub profile URL first.";
    return;
  }
  ...
  try {
    ...
    if (!Array.isArray(repos) || repos.length === 0) {
      githubStatus.textContent = "No public repos found.";
      return;
    }
    ...
    const lines = repos
      ...
      .slice(0, 6)
      .map(repo => {
        const desc = repo.description || "GitHub project";
        return `${repo.name} | ${desc} | ${repo.homepage || repo.html_url}`;
      });

    projectsInput.value = lines.join("\n");
    githubStatus.textContent = "GitHub repos loaded.";
  } catch (err) {
    console.error(err);
    githubStatus.textContent = "Failed to fetch GitHub repos.";
  }
}
fetchGithubBtn.addEventListener("click", fetchGithubRepos);
...
function inferSummary(text) {
  return extractSection(text, ["about", "summary"]) || cleanLines(text).slice(2, 6).join(" ");
}
...
function parseProjects(text) {
  return cleanLines(text).map(line => {
    ...
    return { name, description, url: normalizeUrl(url) };
  }).filter(p => p.name);
}
...
function generateHTML(text) {
  ...
  const summary = inferSummary(text) || "Professional summary";
  ...
  const projectsHtml = (projects.length ? projects : [{ name: "Add projects", description: "Use the projects box or GitHub fetch.", url: "" }])
    .map(p => `
      <div class="project">
...
body{
  margin:0;font-family:Inter,Arial,sans-serif;color:#f8fbff;
  background:
...
.project{
  border:1px solid rgba(255,255,255,.12);
  border-radius:18px;padding:18px;background:rgba(255,255,255,.05)
}
...
@media (max-width:800px){.grid,.projects{grid-template-columns:1fr}.wrap{padding:22px 14px}}
</style>
</head>
<body>
<div class="wrap">
...
generateBtn.addEventListener("click", () => {
  ...
  fileNameInput.value = `${inferName(text).toLowerCase().replace(/\s+/g, "-") || "portfolio-ai"}.html`;
...
B.Tech in Computer Science`;
  ...
  projectsInput.value = "";
  githubStatus.textContent = "Sample loaded. Click Fetch GitHub Repos if needed.";
});

/* Init from share hash */
(function init() {
  const m = location.hash.match(/^#share=(.+)$/);
  if (m) {
    const decoded = decodeHtmlFromHash(decodeURIComponent(m[1]));
    if (decoded) {
